...
namespace Validation
{
    public class InputValidation
    {
        ...
        #endregion
        public bool IsValidAadharCard(String AadharCard)
        {

            string pattern = @"\[0-9]{12}";

            System.Text.RegularExpressions.Match match = Regex.Match(AadharCard.Trim(), pattern, RegexOptions.IgnoreCase);
        ...
        {
            Regex objPatternnumber = new Regex(@"^-?(?:\d+|\d{1,3}(?:,\d{3})+)?(?:\.\d+)?");
