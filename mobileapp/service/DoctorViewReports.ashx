...
<%@ WebHandler Language="C#" Class="DoctorViewReports" %>
...
public class DoctorViewReports : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest (HttpContext context)
    {
        ...
        switch (Action)
        {
                ...
                if (ds != null && ds.Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < ds.Rows.Count; j++)
                    {
                        if (j == 0)
                        {
                            ...
                            sbData.Append("\"sTestDate\":\"" + cGeneralHelper.JSONEscape(ds.Rows[j]["sTestDate"].ToString()) + "\",");
            ...
            case "GetUserReportID":
                string sBooklabtestid = Convert.ToString(context.Request.QueryString["Booklabtestid"]);


                DataTable dst = GetUserReportID(sBooklabtestid);
                if (dst != null && dst.Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < dst.Rows.Count; j++)
                    {
                        ...
                        {
                            sbData.Append("{");
                            sbData.Append("\"sSharedReportId\":\"" + cGeneralHelper.JSONEscape(dst.Rows[j]["sSharedReportId"].ToString()) + "\",");
                            sbData.Append("\"sPatientId\":\"" + cGeneralHelper.JSONEscape(dst.Rows[j]["sPatientId"].ToString()) + "\",");
                            sbData.Append("\"sDoctorId\":\"" + cGeneralHelper.JSONEscape(dst.Rows[j]["sDoctorId"].ToString()) + "\",");
                            sbData.Append("\"sReportId\":\"" + cGeneralHelper.JSONEscape(dst.Rows[j]["sReportId"].ToString()) + "\",");
                            sbData.Append("\"sShared\":\"" + cGeneralHelper.JSONEscape(dst.Rows[j]["sShared"].ToString()) + "\"");
    ...
    public DataTable GetUserDetails(string booklabtestid)
    {
        try
        {
            ...
            string Query = @"select bl.sBookLabId,  bl.sPatientId, bl.steststatus, bl.spaymentstatus, blt.sapprovalstatus, 
		au.sFullName as sPatient,  au.sGender as sGender, 
		...
		trv.sLowerLimit, trv.sUpperLimit, trv.sValue, trv.sResult 
		from bookLab bl  full join appUser au on au.sAppUserId=bl.sPatientId 
		 full join appUser aud on aud.sAppUserId=bl.sDoctorId 
		 join labMaster lm on lm.sLabId=bl.sLabId 
		  join bookLabTest blt on blt.sBookLabId=bl.sBookLabId 
		 join test t on t.sTestId=blt.sTestId 
         join testReportValues trv on trv.sBookLabTestId=blt.sBookLabTestId  
		   where

		    bl.steststatus = 'taken' and
 bl.spaymentstatus = 'paid' and 
 blt.sapprovalstatus ='approved' and
 blt.sBookLabTestId=" + booklabtestid + "";
    ...
    public DataTable GetUserReportID(string sReportId)
    {
        try
        {
            DataTable dst = new DataTable();
            string Query = @"select * from  [dbo].[sharedReport] where sReportId="+sReportId+"";
